...
console.log('\n— cards inventados —')
{
  ...
  check('meses, contíguos, com o vazio no meio', r.months.map(m => m.key), ['2026-12', '2027-01', '2027-02'])
  check('rótulo em inglês', r.months[0].label, 'Dec 2026')
  check('dezembro tem o card a', r.months[0].total.orders, 1)
  check('janeiro tem o card b (virada de ano)', r.months[1].total.orders, 1)
  check('janeiro: o mês vazio? não — b cai em jan; fev tem c (20/out+120=17/fev)', r.months[2].total.orders, 1)
  check('card sem relógio contado à parte', r.withoutClock, 1)
  check('e fora do total', r.total.orders, 3)
  check('peças do card sem relógio não entram', r.total.pieces, 310)
  check('compra em US$', round2(r.total.purchaseUsd), 560)
  check('venda em R$', round2(r.total.saleBrl), 5050)
  check('nada depois do horizonte', r.later, null)

  const soDeqi = arrivalsByMonth(cards, rows, 'd')
  check('filtro por fornecedor tira a Sconcept', soDeqi.total.orders, 2)
  check('e o mês dela some se ficou vazio no fim', soDeqi.months.map(m => m.key), ['2026-12', '2027-01', '2027-02'])
}
    ...
    with anc as (
      select c.id,
             case when s.short_name = 'Sconcept'
                  ...
                  else coalesce(c.sample_approved_at, c.order_confirmed_at) end as ancora
        from cards c left join suppliers s on s.id = c.supplier_id
       where c.board = 'orders' and c.archived = false)
    select to_char(ancora + 120, 'YYYY-MM') as mes, count(*)::int as pedidos,
  ...
  const { rows: pedidosSql } = await db.query(`
    with anc as (
      select c.id,
       ...
       where c.board = 'orders' and c.archived = false)
    select to_char(ancora + 120, 'YYYY-MM') as mes, count(*)::int as pedidos
      from anc where ancora is not null group by 1 order by 1`)
